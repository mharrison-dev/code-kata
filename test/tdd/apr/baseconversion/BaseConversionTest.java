...
class BaseConversionTest {
    ...
    @ParameterizedTest
    @ValueSource(ints = {37, 40})
    public void shouldThrowException_whenInitialBaseIsLargerThanThirtySix(int initialBase) {
        String value = "0";
        int finalBase = 10;

        String expectedExceptionMessage = "Initial base cannot be larger than thirty-six.";
        Exception actualException = assertThrows(
                IllegalArgumentException.class,
                () -> baseConversion.convert(value, initialBase, finalBase)
        );
    ...
    @ParameterizedTest
    @ValueSource(ints = {-1, 0, 1})
    public void shouldThrowException_whenFinalBaseIsSmallerThanTwo(int finalBase) {
        String value = "0";
        int initialBase = 10;

        String expectedExceptionMessage = "Final base cannot be smaller than two.";
        Exception actualException = assertThrows(
                IllegalArgumentException.class,
                () -> baseConversion.convert(value, initialBase, finalBase)
        );
    ...
    @ParameterizedTest
    @ValueSource(ints = {37, 40})
    public void shouldThrowException_whenFinalBaseIsLargerThanThirtySix(int finalBase) {
        String value = "0";
        int initialBase = 10;

        String expectedExceptionMessage = "Final base cannot be larger than thirty-six.";
        Exception actualException = assertThrows(
                IllegalArgumentException.class,
                () -> baseConversion.convert(value, initialBase, finalBase)
        );
